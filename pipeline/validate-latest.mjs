...
const sourceIds = new Set(sources.map((player) => player.id));
const coveredPlayers = players.filter((player) => sourceIds.has(player.id));
const invalidPlayers = coveredPlayers.filter((player) => {
  return (player.singles?.length || 0) + (player.doubles?.length || 0) === 0;
});

if (coveredPlayers.length !== sources.length) {
...
console.log(`Validated latest ranking data for ${coveredPlayers.length} players.`);
